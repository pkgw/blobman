...
//!
//! A blob is just a file whose contents the framework does not care about,
//! except that blobman’s job is to ensure that the contents are exactly what
//! the caller expects.
...
pub struct Session<'a> {
    pub(crate) services: SessionServices<'a>,
    manifest: manifest::Manifest,
    manifest_path: Option<PathBuf>,
    manifest_modified: bool,
}

/// Services provided by the live session.
///
/// This type basically exists to make the borrow checker happy when we
/// delegate mutation-y operations to collections.
pub struct SessionServices<'a> {
    pub(crate) config: &'a config::UserConfig,
    pub(crate) nbe: &'a mut (dyn notify::NotificationBackend + Send),
    pub(crate) digest_table: blobs::DigestTable,
    pub(crate) http_client: reqwest::Client,
}

impl<'a> Session<'a> {
    /// Create and return a new Session.
    pub fn new(config: &'a config::UserConfig, nbe: &'a mut (dyn NotificationBackend + Send)) -> Result<Self> {
        let mut digest_table = blobs::DigestTable::new();
        let (manifest, manifest_path) = manifest::Manifest::find(&mut digest_table)?;
    ...
    pub fn rewrite_manifest(&mut self) -> Result<()> {
        ...
        let text = toml::ser::to_string_pretty(&self.manifest.clone_serde(&self.services.digest_table))?;
    ...
    /// Get the named collection in the manifest, if it exists.
    pub fn get_collection(&self, name: &str) -> Option<&Collection> {
        self.manifest.collections.get(name)
    }

    /// Add the named collection to the manifest, possibly returning
    /// the preexisting collection that was registered under that name.
    pub fn insert_collection(&mut self, coll: Collection) -> Option<Collection> {
        self.manifest.collections.insert(coll.name.clone(), coll)
    }

    /// Add a new item to a collection, retrieving its contents.
    pub async fn insert_item(&mut self, coll_name: &str, item_spec: Table) -> Result<BlobId> {
        let coll = self.manifest.collections.get_mut(coll_name).ok_or(err_msg_value!("no such collection \"{}\"", coll_name))?;
        let id = coll.insert_item(item_spec, &mut self.services).await?;
        self.manifest_modified = true;
        Ok(id)
    }
}

impl<'a> SessionServices<'a> {
    ...
    /// Get a URL using the session-wide HTTP client
    pub async fn get_url(&mut self, url: &str) -> Result<reqwest::Response> {
